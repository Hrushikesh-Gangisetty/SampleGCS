...
data class CompassProgress(
    ...
    val completionMask: List<UByte>, // This is a list in MAVLink message
...
data class CompassCalibrationUiState(
    ...
    val compassProgress: Map<Int, CompassProgress> = emptyMap(), // compass_id -> progress data
    val compassReports: Map<Int, CompassReport> = emptyMap(), // compass_id -> report data
    val isConnected: Boolean = false,
    val showCancelDialog: Boolean = false,
    val showAcceptDialog: Boolean = false,
    val overallProgress: Int = 0, // Overall progress percentage (0-100)
    val calibrationComplete: Boolean = false // True when all compasses report completion
)
